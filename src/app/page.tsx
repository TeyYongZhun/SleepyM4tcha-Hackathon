...
const FEATURES = [
  ...
  },
  {
    ...
    body: "Works with FedEx, DHL, UPS, ocean carriers, freight forwarders and customs brokers alike.",
  },
];
    ...
    <div className="flex flex-1 flex-col bg-paper text-ink">
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-paper/90 px-5 py-5 backdrop-blur sm:px-8 lg:px-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="rounded-lg bg-btn px-5 py-2.5 text-[14.5px] font-semibold text-on-btn transition hover:bg-btn-hover"
          >
      ...
      <main>
          ...
          <div className="flex-1">
            ...
            <div className="mt-9 flex flex-wrap items-center gap-x-5 gap-y-3">
              {signedIn ? (
                <Link
                  href="/dashboard"
                  className="rounded-lg bg-btn px-[22px] py-[13px] text-[15px] font-semibold text-on-btn transition hover:bg-btn-hover"
                >
                  Go to dashboard
                </Link>
              ) : (
        ...
        <p className="mx-auto max-w-[640px] px-5 pb-[72px] text-center text-sm text-ink-soft">
          Works with mail from FedEx, DHL, UPS, Maersk, Evergreen, customs brokers and freight
          forwarders, with no special integration required.
        </p>

        <section id="how-it-works" className="scroll-mt-4 bg-navy px-5 py-20 sm:px-8 lg:px-16">
          <div className="mx-auto max-w-[1200px]">
            <h2 className="text-[32px] font-semibold text-white">How it works</h2>
            <ol className="mt-11 grid gap-7 md:grid-cols-3">
              {STEPS.map((s, i) => (
                <li key={s.title} className="rounded-[14px] bg-navy-2 p-7">
                  <span className="mb-[18px] inline-flex h-[30px] w-[30px] items-center justify-center rounded-full bg-copper text-[13px] font-bold text-white">
                    {i + 1}
                  </span>
        ...
        <section id="features" className="mx-auto max-w-[1200px] scroll-mt-4 px-5 py-20 sm:px-8 lg:px-16">
          ...
          <div className="mt-10 grid gap-[22px] sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map(({ icon: Icon, title, body }) => (
              <div key={title} className="rounded-[14px] border border-line p-6">
                <span className="mb-4 flex h-[38px] w-[38px] items-center justify-center rounded-[10px] bg-paper-2 text-copper">
        ...
        <section className="bg-copper px-5 py-16 text-center sm:px-8">
      ...
      <footer className="flex items-center justify-between border-t border-line px-5 py-8 sm:px-8 lg:px-16">
        <Logo size="sm" />
